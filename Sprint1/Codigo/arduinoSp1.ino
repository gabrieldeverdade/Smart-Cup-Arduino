...
const static double loopDuration = 500.0;
...
void loop() {
  ...
  if (currentTime - lastInterruptionTime > 250) {
    int potenciometerRead = analogRead(potenciometerPin);
    int selectedFlow = map(potenciometerRead, 0, 1023, 0, 30);

    double selectedFlowFrequency = selectedFlow / 7.5;
    double ciclesSurpassed = selectedFlowFrequency * 0.25;
